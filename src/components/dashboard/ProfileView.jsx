...
export const ProfileView = ({ showToast }) => {
    ...
    return (<div className="space-y-12">
      ...
      {error && (<div className="p-4 bg-red-950/40 border border-red-500/30 text-red-400 text-xs rounded font-body" role="alert">
          {error}
        </div>)}
      ...
      <form onSubmit={handleSubmit} className="grid grid-cols-1 lg:grid-cols-12 gap-12">
        ...
        <div className="lg:col-span-4 flex flex-col items-center lg:items-start gap-8">
          ...
          <div className="flex flex-col items-center gap-4 bg-[#0D1626]/30 border border-[rgba(201,162,39,0.12)] rounded-lg p-6 w-full shadow-[0_10px_25px_rgba(0,0,0,0.2)]">
            ...
            <div className="flex gap-3 w-full mt-2">
              <button type="button" onClick={handleUploadClick} className="flex-grow bg-[#C9A227]/10 hover:bg-[#C9A227]/20 border border-[#C9A227]/30 hover:border-[#C9A227] text-[#C9A227] py-2 px-3 rounded font-display text-[9px] tracking-[0.1em] uppercase transition-all duration-300 cursor-pointer text-center">
                Upload
              </button>
              <button type="button" onClick={handleRemoveClick} className="flex-grow bg-red-950/10 hover:bg-red-950/20 border border-red-500/20 hover:border-red-500/50 text-red-400 py-2 px-3 rounded font-display text-[9px] tracking-[0.1em] uppercase transition-all duration-300 cursor-pointer text-center">
                Remove
              </button>
            </div>
        ...
        <div className="lg:col-span-8 bg-[#0D1626]/40 border border-[rgba(201,162,39,0.12)] rounded-lg p-8 space-y-6 shadow-[0_15px_35px_rgba(0,0,0,0.2)]">
          ...
          <AuthInput id="display-name" label="Display Name" value={name} onChange={(e) => setName(e.target.value)} disabled={saving} placeholder="Your name"/>

          <AuthInput id="email-address" label="Email Address (Read Only)" type="email" value={user?.email || ""} disabled className="opacity-60 cursor-not-allowed bg-black/10"/>

          <AuthInput id="user-role" label="Archive Role (Read Only)" type="text" value={user?.role === "admin" ? "Administrator" : "Member"} disabled className="opacity-60 cursor-not-allowed bg-black/10 capitalize"/>
          ...
          <div className="flex flex-col gap-2 w-full">
            ...
            <textarea id="bio" value={bio} onChange={(e) => setBio(e.target.value.slice(0, 250))} disabled={saving} rows={4} maxLength={250} placeholder="Tell the circle about your literary interests..." className="w-full bg-[rgba(255,255,255,0.03)] border border-[rgba(201,162,39,0.18)] rounded px-4 py-3 text-[#F7F5EE] placeholder:text-[#F7F5EE]/30 font-body text-sm focus:outline-none focus:ring-1 focus:ring-[#C9A227] focus:border-[#C9A227] transition-all duration-300 resize-none"/>
