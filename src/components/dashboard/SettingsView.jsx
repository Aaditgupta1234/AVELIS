...
const Toggle = ({ checked, onChange, label, description }) => {
    return (<div className="flex items-start justify-between w-full gap-6 py-1">
      ...
      <button type="button" onClick={() => onChange(!checked)} className={`relative inline-flex h-5.5 w-10 flex-shrink-0 cursor-pointer rounded-full border-2 border-transparent transition-colors duration-300 ease-in-out focus:outline-none focus:ring-1 focus:ring-[#C9A227] focus:ring-offset-1 focus:ring-offset-[#0D1626] ${checked ? "bg-[#C9A227]" : "bg-white/10"}`} role="switch" aria-checked={checked}>
        <span className={`pointer-events-none inline-block h-4.5 w-4.5 transform rounded-full bg-[#0D1626] border border-white/5 shadow-md transition duration-300 ease-in-out ${checked ? "translate-x-4.5" : "translate-x-0"}`}/>
      </button>
    </div>);
};
    ...
    useEffect(() => {
        ...
        if (saved) {
            try {
                ...
                if (parsed.emailNotif !== undefined)
                    setEmailNotif(parsed.emailNotif);
                if (parsed.readingReminders !== undefined)
                    setReadingReminders(parsed.readingReminders);
                if (parsed.borrowReminders !== undefined)
                    setBorrowReminders(parsed.borrowReminders);
                if (parsed.productUpdates !== undefined)
                    setProductUpdates(parsed.productUpdates);
                if (parsed.profileVisible !== undefined)
                    setProfileVisible(parsed.profileVisible);
                if (parsed.activityVisible !== undefined)
                    setActivityVisible(parsed.activityVisible);
                if (parsed.journalPrivate !== undefined)
                    setJournalPrivate(parsed.journalPrivate);
            }
            catch (e) {
    ...
    return (<div className="space-y-12">
      ...
      <form onSubmit={handleSave} className="grid grid-cols-1 lg:grid-cols-12 gap-8">
          ...
          <div className="bg-[#0D1626]/40 border border-[rgba(201,162,39,0.12)] rounded-lg p-6 sm:p-8 space-y-6 shadow-[0_15px_35px_rgba(0,0,0,0.2)]">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-6">
              <div className="flex flex-col gap-2">
                ...
                <select id="font-family" value={font} onChange={(e) => setFont(e.target.value)} disabled={saving} className="w-full bg-[#07111F] border border-[rgba(201,162,39,0.18)] rounded px-4 py-3 text-[#F7F5EE] font-body text-xs focus:outline-none focus:ring-1 focus:ring-[#C9A227] focus:border-[#C9A227] transition-all cursor-pointer">
              ...
              <div className="flex flex-col gap-2">
                ...
                <select id="reading-width" value={width} onChange={(e) => setWidth(e.target.value)} disabled={saving} className="w-full bg-[#07111F] border border-[rgba(201,162,39,0.18)] rounded px-4 py-3 text-[#F7F5EE] font-body text-xs focus:outline-none focus:ring-1 focus:ring-[#C9A227] focus:border-[#C9A227] transition-all cursor-pointer">
              ...
              <div className="flex flex-col gap-2">
                ...
                <select id="font-zoom" value={zoom} onChange={(e) => setZoom(e.target.value)} disabled={saving} className="w-full bg-[#07111F] border border-[rgba(201,162,39,0.18)] rounded px-4 py-3 text-[#F7F5EE] font-body text-xs focus:outline-none focus:ring-1 focus:ring-[#C9A227] focus:border-[#C9A227] transition-all cursor-pointer">
              ...
              <div className="flex flex-col gap-2">
                ...
                <select id="line-height" value={lineHeight} onChange={(e) => setLineHeight(e.target.value)} disabled={saving} className="w-full bg-[#07111F] border border-[rgba(201,162,39,0.18)] rounded px-4 py-3 text-[#F7F5EE] font-body text-xs focus:outline-none focus:ring-1 focus:ring-[#C9A227] focus:border-[#C9A227] transition-all cursor-pointer">
        ...
        <div className="lg:col-span-5 space-y-8">
          ...
          <div className="bg-[#0D1626]/40 border border-[rgba(201,162,39,0.12)] rounded-lg p-6 sm:p-8 space-y-6 shadow-[0_15px_35px_rgba(0,0,0,0.2)]">
            ...
            <div className="space-y-4">
              <Toggle checked={emailNotif} onChange={setEmailNotif} label="Email Correspondence" description="Receive quarterly journals and review digests."/>
              <div className="h-[1px] bg-[rgba(201,162,39,0.08)]"/>
              <Toggle checked={readingReminders} onChange={setReadingReminders} label="Reading Reminders" description="Be notified to continue your active manuscripts."/>
              <div className="h-[1px] bg-[rgba(201,162,39,0.08)]"/>
              <Toggle checked={borrowReminders} onChange={setBorrowReminders} label="Borrow Reminders" description="Alerts for digital commission deadlines."/>
              <div className="h-[1px] bg-[rgba(201,162,39,0.08)]"/>
              <Toggle checked={productUpdates} onChange={setProductUpdates} label="Exhibits & Commissions" description="Announcements of rare physical hardcovers."/>
            </div>
          </div>
          ...
          <div className="bg-[#0D1626]/40 border border-[rgba(201,162,39,0.12)] rounded-lg p-6 sm:p-8 space-y-6 shadow-[0_15px_35px_rgba(0,0,0,0.2)]">
            ...
            <div className="space-y-4">
              <Toggle checked={profileVisible} onChange={setProfileVisible} label="Public Identity" description="Allow other members to find your archival profile."/>
              <div className="h-[1px] bg-[rgba(201,162,39,0.08)]"/>
              <Toggle checked={activityVisible} onChange={setActivityVisible} label="Publish Reading Log" description="Display your current reads to the collective."/>
              <div className="h-[1px] bg-[rgba(201,162,39,0.08)]"/>
              <Toggle checked={journalPrivate} onChange={setJournalPrivate} label="Immutable Journal Privacy" description="Set your reflections to private by default."/>
            </div>
          </div>
          ...
          <div className="bg-[#0D1626]/40 border border-[rgba(201,162,39,0.12)] rounded-lg p-6 sm:p-8 space-y-6 shadow-[0_15px_35px_rgba(0,0,0,0.2)]">
            ...
            <div className="space-y-4 text-xs">
              ...
              <button type="button" onClick={handleSignOutEverywhere} className="w-full flex items-center justify-center gap-2 border border-red-500/20 hover:border-red-500/50 text-red-400 hover:text-red-300 py-3 rounded font-display text-[9px] tracking-[0.15em] uppercase transition-all duration-300 bg-red-950/5 hover:bg-red-950/10 cursor-pointer">
        ...
        <div className="lg:col-span-12 pt-6 border-t border-[rgba(201,162,39,0.1)] flex justify-end">
          <AuthButton type="submit" loading={saving} disabled={saving} className="w-full sm:w-auto sm:px-10">
            Save Preferences
          </AuthButton>
        </div>
      </form>
