...
export const SectionTitle = ({ title, subtitle, align = "center", className, ...props }) => {
    return (<motion.div initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true, margin: "-100px" }} transition={{ duration: 0.6 }} className={cn("flex flex-col gap-4", {
            "text-left": align === "left",
            "text-center items-center": align === "center",
            "text-right items-end": align === "right",
        }, className)} {...props}>
      {subtitle && (<span className="text-gold-primary uppercase tracking-[0.15em] font-semibold text-xs font-inter">
          {subtitle}
        </span>)}
