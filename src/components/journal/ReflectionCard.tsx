...
export const ReflectionCard = ({ reflection, isFeatured = false }: ReflectionCardProps) => {
  ...
  return (
    <motion.article
      tabIndex={0}
      role="article"
      aria-label={`Reflection: ${reflection.title}`}
      whileHover={{ 
        y: -4,
        ...
        <>
          ...
          {reflection.coverImage && (
            <div className="w-full md:w-1/3 aspect-[3/4] relative overflow-hidden group rounded-xl border border-outline-variant/10 flex-shrink-0 bg-surface-container-lowest">
              ...
              <motion.img
                alt={`Cover of ${reflection.bookTitle}`}
        ...
        <>
          ...
          <div>
            <div className="flex items-center gap-3 mb-6">
              <span 
                className={`material-symbols-outlined text-sm select-none ${isPrivate ? "text-on-surface-variant/60" : "text-primary"}`}
