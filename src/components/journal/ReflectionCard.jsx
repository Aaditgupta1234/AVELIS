...
export const ReflectionCard = ({ reflection, isFeatured = false }) => {
    ...
    return (<motion.article tabIndex={0} role="article" aria-label={`Reflection: ${reflection.title}`} whileHover={{
            y: -4,
            borderColor: "rgba(201, 162, 39, 0.3)",
            boxShadow: "0 12px 30px -10px rgba(201, 162, 39, 0.08)"
        }} className={`card-hover focus:outline-none focus:ring-2 focus:ring-primary/40 focus:border-primary/40 transition-all duration-300 rounded-2xl border h-full ${isFeatured
      ...
      {isFeatured ? (<>
          ...
          {reflection.coverImage && (<div className="w-full md:w-1/3 aspect-[3/4] relative overflow-hidden group rounded-xl border border-outline-variant/10 flex-shrink-0 bg-surface-container-lowest">
              <div className="absolute inset-0 bg-primary/5 mix-blend-overlay group-hover:opacity-0 transition-opacity z-10"/>
              <motion.img alt={`Cover of ${reflection.bookTitle}`} src={reflection.coverImage} loading="lazy" whileHover={{ scale: 1.03 }} transition={{ duration: 0.7, ease: easeOut }} className="w-full h-full object-cover"/>
            </div>)}
          ...
          <div className="flex-grow flex flex-col justify-between">
            <div>
              <div className="flex flex-wrap items-center gap-3 mb-4 text-primary">
                ...
                <span className="font-body text-[10px] tracking-[0.2em] uppercase font-bold">Public Reflection</span>
          ...
          <div>
            <div className="flex items-center gap-3 mb-6">
              <span className={`material-symbols-outlined text-sm select-none ${isPrivate ? "text-on-surface-variant/60" : "text-primary"}`} style={{ fontVariationSettings: isPrivate ? "'FILL' 1" : "'FILL' 0" }}>
                {isPrivate ? "lock" : "public"}
              </span>
              <span className={`font-body text-[10px] tracking-[0.2em] uppercase font-bold ${isPrivate ? "text-on-surface-variant/60" : "text-primary"}`}>
                {isPrivate ? "Private Entry" : "Public"}
              </span>
            </div>
          ...
          <div className="pt-6 mt-6 border-t border-outline-variant/10 flex flex-col gap-3">
            {reflection.bookTitle && (<div className="flex items-center gap-2 text-on-surface-variant/50 font-body text-[10px] tracking-wider font-semibold uppercase">
                <span className="material-symbols-outlined text-sm select-none">menu_book</span>
                <span>Linked: {reflection.bookTitle} {reflection.bookAuthor ? `(${reflection.bookAuthor})` : ""}</span>
              </div>)}
            
            <div className="flex justify-between items-center">
              {!isPrivate && reflection.readingTime && (<span className="text-primary font-body text-[10px] tracking-wider font-bold uppercase">
                  {reflection.readingTime}
                </span>)}
