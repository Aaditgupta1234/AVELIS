...
export const AuthorSection = () => {
    ...
    return (<section className="px-margin-mobile md:px-gutter max-w-container-max mx-auto mb-24 md:mb-40">
      <div className="flex items-end justify-between mb-16">
        ...
        <motion.button whileHover={{ color: "var(--color-on-background)", borderBottomColor: "var(--color-on-background)" }} className="text-primary font-display text-[10px] tracking-[0.2em] uppercase border-b border-primary/30 pb-1 transition-colors">
          ALL AUTHORS
        </motion.button>
      </div>

      <motion.div variants={containerVariants} initial="hidden" whileInView="visible" viewport={{ once: true, margin: "-50px" }} className="grid grid-cols-2 md:grid-cols-4 lg:grid-cols-6 gap-8 md:gap-12">
        {authorsData.map((author) => (<motion.div key={author.id} variants={cardVariants} whileHover="hover" tabIndex={0} role="button" aria-label={`View works by ${author.name}`} className="text-center group cursor-pointer focus:outline-none focus:ring-2 focus:ring-primary/40 rounded-xl p-2 transition-all duration-300">
            {/* Avatar Container */}
            <div className="relative w-full aspect-square rounded-full overflow-hidden mb-6 border border-outline-variant/10 group-hover:border-primary/50 group-hover:shadow-[0_0_25px_rgba(201,162,39,0.25)] transition-all duration-500 bg-surface-container-lowest shadow-lg">
              <motion.img alt={author.name} src={author.avatar} loading="lazy" variants={{
                hover: { scale: 1.05 }
            }} transition={{ duration: 0.5, ease: easeOut }} className="w-full h-full object-cover"/>
