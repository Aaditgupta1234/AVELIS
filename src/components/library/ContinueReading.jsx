...
export const ContinueReading = () => {
    ...
    return (<section className="px-margin-mobile md:px-gutter max-w-container-max mx-auto mb-24 md:mb-40">
      <div className="flex items-end justify-between mb-12">
        ...
        <motion.a whileHover={{ color: "var(--color-on-background)", borderBottomColor: "var(--color-on-background)" }} href="#" className="text-primary font-display text-[10px] tracking-[0.2em] uppercase border-b border-primary/30 pb-1 transition-colors">
          VIEW ALL PROGRESS
        </motion.a>
      </div>
      ...
      <motion.div variants={containerVariants} initial="hidden" whileInView="visible" viewport={{ once: true, margin: "-50px" }} className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
        {continueReadingData.map((item) => (<motion.div key={item.id} variants={cardVariants} whileHover={{
            ...
            <div className="w-24 h-36 flex-shrink-0 rounded-lg overflow-hidden shadow-lg group-hover:scale-105 transition-transform duration-500 bg-surface-container-lowest">
              <img alt={`${item.title} cover`} src={item.coverImage} loading="lazy" className="w-full h-full object-cover"/>
            </div>
            ...
            <div className="flex flex-col justify-between py-1 flex-1">
              ...
              <div className="space-y-3">
                ...
                <div className="h-1.5 w-full bg-surface-container-highest rounded-full overflow-hidden">
                  <motion.div initial={{ width: 0 }} whileInView={{ width: `${item.progress}%` }} viewport={{ once: true }} transition={{ duration: 1, ease: easeOut }} className="h-full bg-primary"/>
                </div>

                <motion.button whileHover={{ x: 3 }} aria-label={`Resume reading ${item.title}`} className="text-primary font-body text-[10px] tracking-widest font-bold flex items-center gap-1.5 hover:text-white transition-colors focus:outline-none focus:ring-1 focus:ring-primary/40 rounded px-1">
                  Resume Reading{" "}
