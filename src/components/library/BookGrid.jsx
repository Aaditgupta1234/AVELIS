...
export const BookGrid = ({ books, viewMode }) => {
    ...
    return (<section className="px-margin-mobile md:px-gutter max-w-container-max mx-auto mb-24 md:mb-40">
      <div className="flex items-end justify-between mb-12">
        ...
        <div className="flex gap-4">
          <motion.button whileHover={{ scale: 1.05, borderColor: "rgba(201, 162, 39, 0.5)", color: "var(--color-primary)" }} whileTap={{ scale: 0.95 }} className="w-10 h-10 flex items-center justify-center border border-outline-variant/20 rounded-full text-on-surface transition-all focus:outline-none focus:ring-1 focus:ring-primary/40" aria-label="Previous page">
            <span className="material-symbols-outlined text-lg">chevron_left</span>
          </motion.button>
          <motion.button whileHover={{ scale: 1.05, borderColor: "rgba(201, 162, 39, 0.5)", color: "var(--color-primary)" }} whileTap={{ scale: 0.95 }} className="w-10 h-10 flex items-center justify-center border border-outline-variant/20 rounded-full text-on-surface transition-all focus:outline-none focus:ring-1 focus:ring-primary/40" aria-label="Next page">
            <span className="material-symbols-outlined text-lg">chevron_right</span>
          </motion.button>
        </div>
      </div>

      {books.length === 0 ? (<motion.div initial="hidden" animate="visible" variants={revealVariants.A} className="text-center py-20 border border-white/5 rounded-2xl bg-surface-container-lowest/10">
        ...
        </motion.div>) : (<motion.div variants={containerVariants} initial="hidden" whileInView="visible" viewport={{ once: true, margin: "-50px" }} className={isList
