...
const FooterLink = ({ children, href }) => {
    return (<li>
      <motion.a href={href} className="relative inline-block text-white/40 hover:text-primary transition-colors duration-300 focus:outline-none focus:text-primary focus:ring-1 focus:ring-primary/40 rounded px-1" initial="rest" whileHover="hover">
        {children}
...
const SocialIcon = ({ icon }) => {
    return (<motion.span whileHover={{ y: -2, rotate: 3, color: "var(--color-primary)" }} transition={springs.smooth} className="material-symbols-outlined text-primary/60 cursor-pointer">
      {icon}
    </motion.span>);
};
export const Footer = () => {
    const [isFocused, setIsFocused] = useState(false);
    return (<footer className="py-section-padding relative bg-background border-t border-white/5">
      ...
      <div className="max-w-container-max mx-auto px-gutter grid grid-cols-1 lg:grid-cols-4 gap-20">
        ...
        }} transition={springs.smooth} className="flex border-b pb-2">
            <motion.input onFocus={() => setIsFocused(true)} onBlur={() => setIsFocused(false)} animate={{
            opacity: isFocused ? 1 : 0.6,
        }} aria-label="Email address for newsletter" className="bg-transparent border-none outline-none focus:outline-none focus:ring-0 text-xs w-full text-white placeholder:text-white/20 uppercase tracking-widest transition-opacity duration-300" placeholder={isFocused ? "" : "EMAIL ADDRESS"} type="email"/>
            <button aria-label="Subscribe to newsletter" className="material-symbols-outlined text-primary hover:text-white transition-colors focus:outline-none focus:ring-1 focus:ring-primary/40 rounded px-1">
              chevron_right
            </button>
          </motion.div>
        </div>
