    ...
    return (<AnimatedSection variant="B" className="px-gutter max-w-container-max mx-auto mb-24">
      <div className={`glass-panel border flex flex-col md:flex-row gap-4 items-center justify-between p-6 relative z-10 transition-all duration-300 ${isFocused ? 'border-primary shadow-[0_0_15px_rgba(212,175,55,0.15)]' : 'border-white/10'}`}>
        ...
        <div className="relative w-full md:w-[65%] group">
          <span className={`material-symbols-outlined absolute left-4 top-1/2 -translate-y-1/2 transition-colors duration-300 ${isFocused ? 'text-primary' : 'text-on-background/50'}`} aria-hidden="true">
            search
          </span>
          <input className={`w-full bg-transparent border-b py-3 pl-12 pr-4 font-body text-on-background outline-none transition-all duration-300 ${isFocused ? 'border-primary placeholder:text-on-background/30' : 'border-white/10 placeholder:text-on-background/50'}`} placeholder="Search the archive..." type="text" aria-label="Search the archive" value={query} onChange={(e) => setQuery(e.target.value)} onFocus={() => setIsFocused(true)} onBlur={() => setIsFocused(false)}/>
        </div>
