...
const Particles = () => {
    ...
    return (<div className="absolute inset-0 overflow-hidden pointer-events-none z-0">
      {particles.map((_, i) => (<motion.div key={i} className="absolute w-1 h-1 bg-white/20 rounded-full" initial={{
                x: `${Math.random() * 100}%`,
                y: `${Math.random() * 100}%`,
                opacity: 0,
            }} animate={{
...
export const Hero = () => {
    ...
    return (<section className="min-h-screen flex items-center pt-20 overflow-hidden px-12 relative z-10" onMouseMove={handleMouseMove}>
      ...
      <div className="max-w-[1280px] mx-auto w-full grid grid-cols-1 lg:grid-cols-2 gap-12 items-center relative z-10">
        ...
        }} whileTap={{ scale: 0.98 }} transition={springs.buttonClick} className="border border-primary/30 text-white px-10 py-4 font-display text-[11px] tracking-[0.2em] uppercase transition-colors cursor-pointer focus:outline-none focus:ring-2 focus:ring-primary/40 rounded-lg">
              The Experience
            </motion.button>
          </motion.div>
        </motion.div>
