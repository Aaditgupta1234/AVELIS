...
console.log('Update Book Route Integration Testing Suite\n===========================================');
...
async function runTests() {
  ...
  if (!routeLayer) {
    console.log('FAIL: PATCH /:id route not found in bookRouter.');
    return;
  }
  console.log('PASS: PATCH /:id route is registered in bookRouter.');
  ...
  if (
    ...
    handlerNames[2] === 'updateBookValidator' &&
    handlerNames[3] === 'updateBook'
  ) {
  ...
  try {
    ...
    author = await prisma.author.create({
      data: { fullName: 'Route Test Author', biography: 'Bio' }
    });
    category = await prisma.category.create({
      data: { name: 'Route Test Category ' + Date.now(), description: 'Description' }
    });

    testBook = await prisma.book.create({
      data: {
        title: 'Original Title',
        isbn: 'TEST-ROUTE-ISBN-' + Date.now(),
    ...
    // Test Case 3: Invalid Payload gets 400 Bad Request (rejected by updateBookValidator)
    {
      const req = createMockReq({
        headers: { authorization: `Bearer ${adminToken}` },
        params: { id: testBook.id },
        body: { sellingPrice: -10.00 } // Invalid selling price (must be >= 0)
      });
      const res = createMockRes();

      try {
        const resultRes = await executeRouteStack(req, res, stack);
        if (resultRes.statusCode === 400 && resultRes.body && resultRes.body.success === false && resultRes.body.message === 'Validation failed.') {
          console.log('PASS: Test Case 3: Invalid payload rejected with 400 Bad Request by validator.');
        } else {
    ...
    // Test Case 4: Authenticated Admin gets 200 OK and updated book data
    {
      const req = createMockReq({
        headers: { authorization: `Bearer ${adminToken}` },
        params: { id: testBook.id },
        body: { title: 'Title Updated Via Integrated Route' }
      });
      const res = createMockRes();
      ...
      try {
        ...
        if (
          ...
          resultRes.body.message === 'Book updated successfully.' &&
          resultRes.body.data.title === 'Title Updated Via Integrated Route'
        ) {
          console.log('PASS: Test Case 4: Authenticated Admin successfully updated the book and received standard 200 response.');
        } else {
    ...
    console.error('Unexpected error in route tests:', err);
  } finally {
    // Cleanup
    if (testBook) {
